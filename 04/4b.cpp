...
int main() {
  ...
  int last_winner;
  int *complete_score = new int[boardList.size()]{0};

  for (int nextValue : drawValues) {
    int it = 0;
    for (Board &b : boardList) {
      if (b.markValue(nextValue) && b.isComplete()) {
        int score = b.sumOfUnmarked() * nextValue;
        if (complete_score[it] == 0) {
          complete_score[it] = 1;
          last_winner = score;
        }
      }
  ...
  delete complete_score;

  std::cout << last_winner << std::endl;
